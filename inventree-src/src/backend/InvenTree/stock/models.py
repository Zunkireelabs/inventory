...
class StockItem(
    ...
    def _create_serial_numbers(cls, serials: list, **kwargs) -> QuerySet:
        ...
        if 'part' not in data:
            raise ValidationError({'part': _('Product must be specified')})
    ...
    def clean(self):
        ...
        try:
            ...
            if self.part.trackable and self.quantity != int(self.quantity):
                raise ValidationError({
                    'quantity': _('Quantity must be integer value for trackable products')
                })

            # Virtual parts cannot have stock items created against them
            if self.part.virtual:
                raise ValidationError({
                    'part': _('Stock item cannot be created for virtual products')
                })
        ...
        if self.build:
            if self.part == self.build.part:
                ...
                raise ValidationError({
                    'build': _('Build reference does not point to the same product object')
                })
    ...
    supplier_part = models.ForeignKey(
        ...
        help_text=_('Select a matching supplier product for this stock item'),
        related_name='stock_items',
    )
    ...
    def convert_to_variant(self, variant, user, notes=None):
        ...
        self.add_tracking_entry(
            ...
            notes=_('Converted to product') + ': ' + variant.full_name,
        )
    ...
    def serializeStock(
        ...
        if not self.part.trackable:
            raise ValidationError({'part': _('Product is not set as trackable')})
        ...
        try:
            ...
            if other:
                ...
                if self.part != other.part:
                    raise ValidationError(_('Stock items must refer to the same product'))
                ...
                if (
                    ...
                    raise ValidationError(
                        _('Stock items must refer to the same supplier product')
                    )
