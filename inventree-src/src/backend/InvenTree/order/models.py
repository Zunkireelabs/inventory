    ...
    def add_line_item(
        ...
        * The quantity is greater than zero
        ...
        if supplier_part.supplier != self.supplier:
            raise ValidationError({
                'supplier': _('Product supplier must match PO supplier')
            })
    ...
    def receive_line_items(
        ...
        for line_item in line_item_map.values():
            ...
            if not line_item.part or not line_item.part.part:
                raise ValidationError({_('Line item is missing a linked product')})
        ...
        for item in items:
            ...
            if base_part.virtual:
                ...
                if serialize:
                    raise ValidationError(
                        _('Serial numbers cannot be assigned to virtual products')
                    )
    ...
    def clean(self) -> None:
        ...
        if self.order.supplier and self.part:
            ...
            if self.part.supplier != self.order.supplier:
                raise ValidationError({'part': _('Supplier product must match supplier')})
        ...
        if self.build_order:
            ...
            if part:
                ...
                if self.build_order.part != self.part.part:
                    raise ValidationError({
                        'build_order': _('Build order product must match line item product')
                    })
    ...
    def clean(self) -> None:
        ...
        if self.part:
            if not self.part.salable:
                raise ValidationError({
                    'part': _('Only salable products can be assigned to a sales order')
                })
...
class SalesOrderAllocation(models.Model):
    ...
    def clean(self):
        ...
        try:
            ...
            errors['line'] = _('Cannot allocate stock to a line without a product')
