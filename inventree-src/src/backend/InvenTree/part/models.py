...
class PartCategory(
    ...
    default_location = TreeForeignKey(
        ...
        help_text=_('Default location for products in this category'),
    )
    ...
    default_keywords = models.CharField(
        ...
        help_text=_('Default keywords for products in this category'),
    )
...
class PartCategoryParameterTemplate(InvenTree.models.InvenTreeMetadataModel):
    ...
    class Meta:
        ...
        verbose_name = _('Product Category Parameter Template')
...
class Part(
    ...
    def check_parameter_delete(self, parameter):
        ...
        if self.locked and get_global_setting('PART_ENABLE_LOCKING'):
            raise ValidationError(_('Cannot delete parameters of a locked product'))

    def check_parameter_save(self, parameter):
        """Custom save check for Parameter instances associated with this Part."""
        if self.locked and get_global_setting('PART_ENABLE_LOCKING'):
            raise ValidationError(_('Cannot modify parameters of a locked product'))
    ...
    def delete(self, **kwargs):
        ...
        """
        if self.locked and get_global_setting('PART_ENABLE_LOCKING'):
            raise ValidationError(_('Cannot delete this product as it is locked'))

        if self.active:
            raise ValidationError(_('Cannot delete this product as it is still active'))

        if not get_global_setting('PART_ALLOW_DELETE_FROM_ASSEMBLY', cache=False):
            if BomItem.objects.filter(sub_part=self).exists():
                raise ValidationError(
                    _('Cannot delete this product as it is used in an assembly')
                )
    ...
    def validate_revision(self):
        ...
        if self.revision_of:
            if self.revision_of == self:
                raise ValidationError({
                    'revision_of': _('Product cannot be a revision of itself')
                })
            ...
            if self.revision_of.is_template:
                raise ValidationError({
                    'revision_of': _('Cannot make a revision of a template product')
                })

            # parent part must point to the same template (via variant_of)
            if self.variant_of != self.revision_of.variant_of:
                raise ValidationError({
                    'revision_of': _('Parent product must point to the same template')
                })
    ...
    def validate_unique(self, exclude=None):
        ...
        if self.IPN and not allow_duplicate_ipn:
            ...
            if parts.exists():
                raise ValidationError({
                    'IPN': _('Duplicate IPN not allowed in product settings')
                })
        ...
        if (
            ...
            raise ValidationError(_('Duplicate product revision already exists.'))
        ...
        if (
            ...
            raise ValidationError(
                _('Product with this Name, IPN and Revision already exists.')
            )
    ...
    def clean(self):
        ...
        if self.category is not None and self.category.structural:
            raise ValidationError({
                'category': _('Products cannot be assigned to structural product categories!')
            })
    ...
    name = models.CharField(
        max_length=100, blank=False, help_text=_('Product name'), verbose_name=_('Name')
    )

    is_template = models.BooleanField(
        default=part_settings.part_template_default,
        verbose_name=_('Is Template'),
        help_text=_('Is this product a template product?'),
    )
    ...
    variant_of = models.ForeignKey(
        ...
        on_delete=models.SET_NULL,
        help_text=_('Is this product a variant of another product?'),
    ...
    description = models.CharField(
        ...
        help_text=_('Product description (optional)'),
    )
    ...
    keywords = models.CharField(
        ...
        help_text=_('Product keywords to improve visibility in search results'),
    )
    ...
    IPN = models.CharField(
        ...
        null=True,
        verbose_name=_('IPN'),
        help_text=_('Internal Product Number'),
    )
    ...
    revision = models.CharField(
        ...
        blank=True,
        null=True,
        help_text=_('Product revision or version number'),
    ...
    revision_of = models.ForeignKey(
        ...
        blank=True,
        on_delete=models.SET_NULL,
        help_text=_('Is this product a revision of another product?'),
    ...
    default_expiry = models.PositiveIntegerField(
        ...
        help_text=_('Expiry time (in days) for stock items of this product'),
    )
    ...
    units = models.CharField(
        ...
        help_text=_('Units of measure for this product'),
    ...
    assembly = models.BooleanField(
        ...
        help_text=_('Can this product be built from other products?'),
    )

    component = models.BooleanField(
        default=part_settings.part_component_default,
        verbose_name=_('Component'),
        help_text=_('Can this product be used to build other products?'),
    )

    trackable = models.BooleanField(
        default=part_settings.part_trackable_default,
        verbose_name=_('Trackable'),
        help_text=_('Does this product have tracking for unique items?'),
    )

    testable = models.BooleanField(
        default=False,
        verbose_name=_('Testable'),
        help_text=_('Can this product have test results recorded against it?'),
    )

    purchaseable = models.BooleanField(
        default=part_settings.part_purchaseable_default,
        verbose_name=_('Purchaseable'),
        help_text=_('Can this product be purchased from external suppliers?'),
    )

    salable = models.BooleanField(
        default=part_settings.part_salable_default,
        verbose_name=_('Salable'),
        help_text=_('Can this product be sold to customers?'),
    )

    active = models.BooleanField(
        default=True, verbose_name=_('Active'), help_text=_('Is this product active?')
    )

    locked = models.BooleanField(
        default=False,
        verbose_name=_('Locked'),
        help_text=_('Locked products cannot be edited'),
    )

    virtual = models.BooleanField(
        default=part_settings.part_virtual_default,
        verbose_name=_('Virtual'),
        help_text=_('Is this a virtual product, such as a software product or license?'),
    )

    consumable = models.BooleanField(
        default=False,
        verbose_name=_('Consumable'),
        help_text=_('Is this product consumable, such as glue or a fastener?'),
    )

    bom_validated = models.BooleanField(
        default=False,
        verbose_name=_('BOM Validated'),
        help_text=_('Is the BOM for this product valid?'),
    )
    ...
    responsible_owner = models.ForeignKey(
        ...
        help_text=_('Owner responsible for this product'),
...
class PartPricing(common.models.MetaMixin):
    ...
    bom_cost_min = InvenTree.fields.InvenTreeModelMoneyField(
        ...
        help_text=_('Minimum cost of component products'),
    )
    ...
    bom_cost_max = InvenTree.fields.InvenTreeModelMoneyField(
        ...
        help_text=_('Maximum cost of component products'),
    )
    ...
    supplier_price_min = InvenTree.fields.InvenTreeModelMoneyField(
        ...
        help_text=_('Minimum price of product from external suppliers'),
    )
    ...
    supplier_price_max = InvenTree.fields.InvenTreeModelMoneyField(
        ...
        help_text=_('Maximum price of product from external suppliers'),
    )
    ...
    variant_cost_min = InvenTree.fields.InvenTreeModelMoneyField(
        ...
        help_text=_('Calculated minimum cost of variant products'),
    )
    ...
    variant_cost_max = InvenTree.fields.InvenTreeModelMoneyField(
        ...
        help_text=_('Calculated maximum cost of variant products'),
    )
...
class PartStocktake(models.Model):
    ...
    part = models.ForeignKey(
        ...
        verbose_name=_('Product'),
        help_text=_('Product for stocktake'),
    )
...
class PartSellPriceBreak(common.models.PriceBreak):
    ...
    class Meta:
        ...
        verbose_name = _('Product Sale Price Break')
...
class PartInternalPriceBreak(common.models.PriceBreak):
    ...
    class Meta:
        ...
        unique_together = ('part', 'quantity')
        verbose_name = _('Product Internal Price Break')
...
class PartTestTemplate(InvenTree.models.InvenTreeMetadataModel):
    ...
    class Meta:
        ...
        verbose_name = _('Product Test Template')
    ...
    def validate_unique(self, exclude=None):
        ...
        if not self.part.testable:
            raise ValidationError({
                'part': _('Test templates can only be created for testable products')
            })
...
class BomItem(InvenTree.models.MetadataMixin, InvenTree.models.InvenTreeModel):
    ...
    def recalculate_quantity(self):
        ...
        try:
            ...
            if not self.sub_part.units and not InvenTree.conversion.is_dimensionless(
                ...
                raise ValidationError({
                    'raw_amount': _('Invalid quantity - no units specified for product')
                })
    ...
    part = models.ForeignKey(
        ...
        verbose_name=_('Product'),
        help_text=_('Select parent product'),
    ...
    sub_part = models.ForeignKey(
        ...
        help_text=_('Select product to be used in BOM'),
    ...
    raw_amount = models.CharField(
        ...
        help_text=_('Amount of sub-product consumed to produce one product'),
        blank=False,
        null=False,
    )
    ...
    inherited = models.BooleanField(
        ...
        help_text=_('This BOM item is inherited by BOMs for variant products'),
    )

    allow_variants = models.BooleanField(
        default=False,
        verbose_name=_('Allow Variants'),
        help_text=_('Stock items for variant products can be used for this BOM item'),
    )
    ...
    def clean(self):
        ...
        try:
            ...
            if self.sub_part:
                ...
                raise ValidationError({'sub_part': _('Sub product must be specified')})
        except Part.DoesNotExist:
            raise ValidationError({'sub_part': _('Sub product must be specified')})
...
class BomItemSubstitute(InvenTree.models.InvenTreeMetadataModel):
    ...
    def validate_unique(self, exclude=None):
        ...
        if self.part == self.bom_item.sub_part:
            raise ValidationError({
                'part': _('Substitute product cannot be the same as the master product')
            })
    ...
    part = models.ForeignKey(
        ...
        verbose_name=_('Product'),
        help_text=_('Substitute product'),
        limit_choices_to={'component': True},
    )
...
class PartRelated(InvenTree.models.InvenTreeMetadataModel):
    ...
    part_2 = models.ForeignKey(
        ...
        help_text=_('Select Related Product'),
    )
    ...
    def clean(self):
        ...
        if self.part_1 == self.part_2:
            raise ValidationError(
                _('Product relationship cannot be created between a product and itself')
            )
